...
- 支持预定义的产品能力
  ...
  一个好的LLMAgent必然要有"与人协同"的能力，比如Agent要求用户权限放过，信息澄清，二次确认，或者用户主动想对任务执行进行引导等等。因此ThinkingAgent需要在任务执行过程中与用户进行各种指令的交互，`PipelineDriver`负责管理暴露给应用入口层的异步消息队列，对用户和Agent之间交互的消息进行收发和传递
...
仅代表个人观点：从产品角度，Skills有用；从技术角度，我觉得Skills是没有意义的过渡性产物，Agent想要对外部能力进行更高质量地集成，重心应直接放在提升大模型本身的能力或者制造一个高质量的推理轨迹上。本着这样一个意识形态，ThinkingAgent的设计遵循了以下的思路：
...
可靠性是让Agent尽量能容错，能继续干活；而安全性是让Agent别瞎干活，该停就停下来。安全性建设是需要伴随产品功能迭代持续进行和完善的工作，而目前ThinkingAgent做的还太嫩了，仅从以下三个角度实施了一些基础性的设计
...
SQL工具的风险是 SQL 注入和数据修改，因此SQL Query Tool的Safeguard定制了如下防御逻辑：
...
- **数据脱敏**：返回数据如果涉及敏感字段，Safeguard会执行脱敏再返回
...
LLM发起一次工具调用后，如果其Safeguard发现这次工具调用相关信息没有出现在权限系统的白名单中，Safeguard会通过Pipeline Driver向用户端发送操作确认消息，收到用户的二次确认后才被允许执行
