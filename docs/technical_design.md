...
> ThinkingAgent 是一个轻量级，准生产就绪的LLM Agent 框架，支持多 LLM Provider、复杂任务编排、自动故障恢复和长任务断点续跑。本文记录了从0到1构建ThinkingAgent过程中自己的一点思考，皆是原创观点，如有雷同，纯属巧合。
...
第二，"代价意识"——不是所有失败都需要最激进的恢复策略。比如说只是工具参数写错了，重试一次即可，没必要让LLM对任务重新进行规划。代价意识让系统在可靠性和效率之间找到平衡
...
首先我将Agent的能力划分成如上图所示的四个层次。每一个层次面向不同的领域问题，并且可以独立深入和演进。同时这四个层次从下到上也代表了Agent能力的进阶路径 。罗马不是一天建成的，第一个版本我给自己设置了一个月的deadline，这种分层可以让我决定在有限的时间内哪些领域可以多花一点精力，哪些少一点；哪些在开始阶段就必须要搞定，哪些可以未来再补充
...
一开始我采用传统数据流系统的设计思路来建模ThinkingAgent。但是，随着系统功能迭代和复杂度上升，原来设计好的聚合边界开始模糊，变化传播范围开始失去约束，加入新的feature越来越难，这让我不得不停下来重新审视自己的设计。LLM Agent与传统数据流系统相比，我觉得最重要的区别就是LLM-in-the-loop，我们需要为LLM这个不确定算子去设计围绕其运行的所谓的Harness框架，从架构层面去支撑LLM的动态决策和行为

调整设计思路后，我开始采用DDD领域建模+数据流系统建模的设计方法：

1. 基于数据的流动和变化，提出系统关键的业务事实
2. 编排这些业务事实发生的时间先后顺序
...
sequenceDiagram
    ...
    LLM Gateway-->>Executor: 返回下一步Decision
    alt Decision==调用工具
...
一个用户提交一个任务后，ThinkingAgent需要自主规划并完成任务，在任务处理过程中需要所有聚合根的参与。为了不让依赖变成难以迭代和维护的网状依赖，Pipeline作为中央协调者成为星形拓扑的唯一汇合点
...
虽然Pipeline是中央协调者，但不意味着其他聚合之间一点依赖关系都没有。Pipeline解决的是Agent软件架构全局性的依赖问题，但如果依赖关系是非常简单的单向依赖，聚合之间可以直接依赖。比如ContextManager这个推理上下文管理器就需要依赖上下文裁剪器和token预估器来解决上下文窗口超过模型限制的问题
...
- 支持预定义的产品能力
  ...
  一个好的LLM Agent必然要有"与人协同"的能力，比如Agent要求用户权限放过，信息澄清，二次确认，或者用户主动想对任务执行进行引导等等。因此ThinkingAgent需要在任务执行过程中与用户进行各种指令的交互，`PipelineDriver`负责管理暴露给应用入口层的异步消息队列，对用户和Agent之间交互的消息进行收发和传递
...
ThinkingAgent使用`EventBus`管理所有事件通知和异步操作，在系统设计阶段就已经定义好了完整的系统领域事件，这些领域事件会被Agent Model通过 `EventBus` 发布，这意味着：
...
ThinkingAgent的推理模式如上图所示：一个任务首先会被拆解成不同的执行步骤Stage，Stage之间有明确的依赖关系，比如上图中Stage3要依赖Stage1和Stage2的输出。每个Stage输出的结果必须流经一个Reflection节点，通过反思机制对结果执行评估。如果评估不通过，会触发Stage的重新规划或者原地重试；如果评估通过就继续按照计划处理后续的Stage。每个Stage内部的工作流程是跑一个ReAct工作循环。总结起来，ThinkingAgent是以**Plan-and-Execute为骨架**，以**ReAct为血肉**，以**Reflection为进化驱动力的混合式智能体**

#### 3.1.2 三段式循环: 系统主体控制流结构

ThinkingAgent的Pipeline流程包含非常多的处理步骤，同时还要考虑所有步骤可能的出错模式和对应的恢复逻辑，复杂度很高。最开始落地时，系统主体的控制循环逻辑一度达到了几千行代码，非常难以维护和迭代。所以一个自然而然的想法就是考虑如何分治，进行关注点分离，经过优化的Pipeline Loop开始采用如下图所示的三级嵌套循环结构
...
- 适配ThinkingAgent的推理模式，解耦控制流，降低决策树深度
- 减少异常恢复逻辑的复杂度，有利于后续对系统做可靠性建设
...
最终的返回被作为知识注入到上下文中。因为本文重点是描述ThinkingAgent的整体设计，因此RAG部分不作详细说明
...
ThinkingAgent有一个周期性任务负责对用户近期的原始偏好Entries进行总结。不同于知识注入，Agent在任务分析阶段就会将总结好的用户偏好信息注入到任务分析上下文中，Analyzer需要基于用户提交的任务信息决定应用哪些偏好，并将被应用的偏好信息转化到任务分析报告里的"执行约束","工具匹配","模型路由提示"等字段中。这些字段内容会被后续的Planner，ModelSelector，QualityEvaluator组件使用，以便在任务执行过程中更好地对齐用户的偏好
...
烂大街的东西不再赘述，这里只着重提几点实践过程中让我有一些感觉的东西

##### 3.3.2.1 从模型行为规律反推 Prompt 布局

在 Agent 场景里，Prompt 不是一次性问答中的提示语，而是贯穿多轮推理、工具调用、错误恢复和结果评估的运行时控制面。随着任务执行时间变长，上下文会不断累积，哪些信息放在前面，哪些信息放在后面，哪些信息需要周期性回灌，会直接影响模型是否还能保持任务目标、执行约束和当前状态的一致性。

一个对长上下文 Agent 很重要的经验现象是：模型对上下文不同位置的信息利用并不均匀。实践中经常能观察到，靠近开头的全局指令和靠近结尾的近期状态更容易影响模型输出，而中间区域的信息在长上下文中更容易被弱化。这个现象可能与训练数据中的文档结构偏好、位置编码带来的距离效应、注意力计算中的相对位置敏感性，以及指令微调后形成的“开头读规则，结尾读状态”的行为模式有关。

基于这个判断，ThinkingAgent 的 Prompt 布局遵循一个简单原则：

- 头部放稳定的全局控制信息：系统角色、任务目标、用户真实意图、不可违反的执行约束、工具使用总规则。
- 中部放可压缩的过程信息：历史阶段结果、工具调用记录、阶段间依赖、已完成工作的摘要。
- 尾部放当前决策所需的高优先级信息：当前 Stage 目标、最近一次工具结果、错误恢复提示、用户最新纠偏、质量评估反馈。

换句话说，头部负责“定方向”，尾部负责“控动作”，中部负责“保连续性”。

举例来说，当某个 Stage 连续多次调用同一个失败工具时，ThinkingAgent 不会修改最前面的系统提示词，而是在上下文尾部插入一条临时纠偏消息：

```text
你已经连续 3 次调用 search 工具且返回结果为空。请先判断当前搜索关键词是否过窄，必要时改写查询；如果问题不依赖实时信息，请基于已有上下文给出阶段性结论。
```

这类提示放在尾部，是因为它只服务于当前局部决策，具有强时效性；而任务目标、输出约束、权限边界等长期约束仍然放在上下文头部，避免在多轮推理中被局部状态覆盖。

当然，这种布局不是对模型内部注意力机制的严格证明，而是一个工程经验假设。不同模型、不同上下文长度、不同 API 消息格式下，位置效应的表现会不一样。因此 ThinkingAgent 并不只依赖Prompt布局本身，还会配合上下文压缩、阶段摘要、质量评估和恢复策略，避免系统可靠性完全绑定在某一种模型行为假设上
...
有很多任务不止是要一个推理答案，还需要额外输出一些东西，比如输出一篇文档。我把这些额外输出的产物命名为推理的副作用。在实践中我发现在具有Self-Reflection的推理模式里，如果不为副作用设计提示词，容易引发结果评估不通过，进而开始触发Agent执行恢复流程。因此需要显式地为这些副作用设计一些提示，比如：
...
一个Agent做Self-Reflection的输入应该定义为从副作用中抽取的特征，而不是副作用内容本身，这种方式效果更稳定也更经济。比如一个任务：
...
ThinkingAgent被设计成需要处理多步骤和具有一定复杂性任务的Agent，这些任务的处理时间往往都是小时级别，最蛋疼的情况是眼看临门一脚，结果因为诸如“推理偏离目标”等原因，任务没有得到完整解决
...
仅代表个人观点：从产品角度看，Skills 这类能力包是有价值的，尤其适合把行业 SOP、工具使用经验和交付模板快速包装成可复用的产品能力。但从ThinkingAgent目前的技术设计取舍看，我更倾向于不把Skills作为一个独立的一等抽象，而是把它拆解到知识、偏好、工具和推理轨迹控制几个更基础的层面里

这样做的原因是：Agent对外部能力的高质量集成，最终依赖的不是某个固定能力包本身，而是系统能否在合适的上下文里找到合适的信息、暴露合适的工具，并引导 LLM 产生稳定的推理和执行轨迹，因此 ThinkingAgent 采用了下面的设计取向：

- 工具要简约，具备好的场景适应性和能力泛化性，就像Shell一样
- 所有SOP，资料，文档等信息等被纳入到“知识”的范畴；人设，风格等被纳入“用户偏好”的范畴
- 工具的按需加载源于Analyzer和Planner的共同决策
- 知识的按需加载源于检索技术+LLM
- 对于单个用户而言，结合时效性考虑，一个人偏好数据的数据量不会很大，因此采用“定期总结，整体加载”的策略

这不是否定 Skills 的产品价值，而是 ThinkingAgent 在当前阶段选择了一种更底层、更少概念负担的能力组织方式
...
处理可靠性最麻烦的点就是它无处不在，每个环节，每次调用都要涉及。而因为大模型的介入，这个问题变得更棘手。你可能没有看到系统出现任何异常和报错，但推理轨迹就是陷入局部死循环，就是偏离目标。常规系统面临的可靠性问题Agent要cover，而大模型引发的**推理可靠性**问题，LLM Agent也同样需要提出对应的处理手段
...
在“3.1.2三段式工作循环”章节有提到说这样的程序控制流结构有利于可靠性处理逻辑的设计，正如上图所示，ThinkingAgent的可靠性逻辑结构与工作循环结构是相辅相成的。每一个横向的切面关注着Pipeline执行的不同层级，也同时关注它要处理异常的层级；而纵向穿插的数轴代表着贯穿所有层次，独立的推理上下文。推理上下文领域的异常有它自身的特点，需要单独思考处理模式。
...
| LLM Gateway   | 关注与不同厂商API交互的异常                                  | - 本地检查LLM回包的完整性<br>- 不同厂商API返回的错误信息     | - 本地修复LLM回包<br>- 带Jitter的Backoff重试<br>- 错误注入上下文由LLM自行处理<br>- 模型降级 |
| Tool Registry | 工具调用的异常                                               | - 调用前检查<br>- 连续重复调用纠偏<br>- 具体工具返回的错误信息 | - 带Jitter的Backoff重试<br>- 自动插入纠偏Prompt<br>- 工具降级<br>- 错误注入上下文由LLM自行处理 |
...
- 在原有错误码基础信息（Category+code+message）之上增加了caller action和action data等作为恢复异常的输入信息，直接驱动调用端的恢复行为
...
```python
# Auth / Permission
AgentNormalizedError.AUTH_FAILED:           (ErrorCategory.AUTH,           CallerAction.SWITCH_MODEL),
...
另一个点就是恢复策略需要有自动升级机制，避免“死磕”。比如ThinkingAgent发现某步骤的执行次数已经超过单步骤最大允许重试次数，此时会在推理上下文中自动插入纠偏引导提示：“对于StepN，你已经进行了XX次重试，超过了系统允许的步骤重试次数，现在需要从其他恢复策略中选择一个你认为最合适的执行，注意选择恢复策略时同样需要考虑#3.1中列举的代价信息和触发条件......”
...
- 语义块(BLOCK)：代表不可再分割的原子语义单元，比如工具的调用和对应的回复消息必须做好配对并放在一个语义块中。ThinkingAgent中的语义块BLOCK有四种，分别是：SYS块，USER块，ASSISTANT块和TOOL块，它们分别对应system prompt，用户prompt，独立的推理assistant消息和工具调用/回复消息

- 同属于一个Stage的BLOCK被划分成一个GROUP

  将消息分成BLOCK和GROUP的目的是为了在推理上下文中制造“信息单元”，这些“信息单元”呈现上下文的相关性和语义完整性，是压缩算法保障推理轨迹不断裂的数据结构基础
...
<img src="/Users/yuwu/Desktop/AILearning/AgentLearning/NanoAgent/docs/assets/image-20260529224935754.png" alt="image-20260529224935754" style="zoom:50%;" />

这个推理轨迹的消息分配和布局有几个特点值得注意：

- 使用 Tool Call消息模拟执行计划的制定和内容展示
...
- 每个Stage开始执行时首先会插入一个user role消息来说明本Stage的执行目标/关键产出/执行约束等信息，同时会再次Callback任务整体的目标/用户的真实意图

一个推理轨迹就是与LLM一次完整对话的过程，ThinkAgent想将这个对话塑造成：

1. 以常规的角色话术开始 -> 描述本次任务各维度的信息 -> 任务整体的规划 -> 单个步骤具体执行策略 -> 单个步骤的执行过程 -> 单个步骤的执行结果 -> [下一个步骤描述] -> ...... ->  任务最后的输出结果
...
【Step3】: 从最后一个Group（图中示意的Group N）的最后一个BLOCK开始倒序扫描，目的是看剩余预算还能保住几个BLOCK。找到开始不能满足预算的第一个BLOCK作为分割点(比如上图中红色虚线箭头)
...
- 渐进式处理——从侵入最少的策略开始，从最安全的处理区域开始。采用渐进式的处理模式，一旦发现目前窗口中token数满足预算要求就早停，越少处理越少破坏
- 不引入直接裁剪策略，因为在实践中发现这种方式可能引发复杂任务的推理轨迹断裂或者歧义，效果不好

#### 4.2.6 Prompt Caching

部分LLM提供商，比如Anthropic, MiniMax, Kimi等等，它允许API调用方显式地主动控制Transformer的KV Cache。这个Prompt Caching技术对于ThinkingAgent这种需要处理多轮次，多步骤，长程任务的Agent来说非常友好，可以节省不少开支。目前ThinkingAgent支持基于cache_control参数控制缓存的方式。
...
<img src="/Users/yuwu/Desktop/AILearning/AgentLearning/NanoAgent/docs/assets/image-20260529065154900.png" alt="image-20260529065154900" style="zoom:50%;" />

上图展示了推理上下文消息LLM Context和一个具体LLM服务提供商Provider API请求与回复之间的相互转化过程

1. ReasoningManager负责将LLM Context转化成标准请求协议UnifiedLLMRequest
2. LLMGateway使用UnifiedLLMRequest调用一个具体LLM Provider
3. LLM Provider将UnifiedLLMRequest转化成自己API需要的请求格式
4. 收到LLM的回包后，进行一个逆向的转化过程
...
所有LLM调用的回包都会被转化成`UnifiedLLMResponse`
...
| 2    | Model Degrade         | Quota Exceeed<br>各类回包解析问题<br>......                  |
| 3    | Model Provider Switch | 4xx Auth/Permission Denied<br>Content Length Error<br>Invalid Request<br>触发Content Policy<br/>.......... |

##### 4.3.3.2 基于报文内容修复

有时没有显示的错误码，但是LLM Provider回复的报文内容不合法，主要有以下两种情况：

- 格式非法：比如Agent明确需要JSON格式，但是LLM返回了JSON fence
- 丢失必需的key-value

如果回包只是格式非法或者丢失一些非必需的key-value，会先用本地JSON schema修复工具处理成合法JSON，让Agent继续回到正确的推理轨迹中。如果回包缺失关键信息，Agent会插入引导提示词，让LLM尝试自己修复

##### 4.3.3.3 总结

LLMGateway本身没有特别多的内容，它的重点是设计一套统一标准协议，对接不同LLM服务提供商的API。目前ThinkingAgent对接了OpenAI，Anthropic，DeepSeek，GLM，Kimi，Qwen，MiniMax这几家厂商的大模型
...
调用工具前先进行工具存在性检查，必填参数检查，参数类型约束检查等准入检查。这部分工作没有交给具体的工具执行时才做，而是由ToolRegistry调用前统一完成，原因有两个：

- 提升效率：有些工具调用是比较重的，比如拉起Python执行环境
- 逻辑复用：准入检查所需要的输入信息由Tool Schema提供，在调用工具前就可以获取到。将这些检查以及对应的修复逻辑收口在Tool Registry层，可以避免在每个Tool里都写一遍

#### 4.4.2 工具降级

如果某些工具在有限次重试后依然调用失败，可以考虑降级方案。比如search联网搜索工具重试失败，可以降级使用数据库相关工具尝试本地搜索
...
ThinkingAgent 的质量评估分三个层次：计划评估、阶段执行情况评估、任务结果评估。这三个层次不是冗余的，每个层次解决不同的问题。

**如果只有任务级评估会怎样？**

任务执行完所有步骤后，评估最终结果。如果不通过，重新执行整个任务。

问题：代价太高。一个 6 步任务，如果第 2 步的结果有问题，但直到最后才发现，剩余4 步的工作都白费了。而且，任务级评估只能告诉你"结果不对"，但不能告诉你"哪一步出了问题"，重新执行时无法针对性地改进。
...
对一个阶段执行情况的评估主要从显式要求和隐式要求两个方面展开：

- 显式要求：“计划步骤要求的key results，真正执行的结果里是否都包含”；“步骤输出的结果满足输出约束要求吗”等等
- 隐式要求：“步骤结果是否真正实现了预设目标，而不仅仅是部分达成？“；”这个步骤的结果是否推动整体 `task goal` 向前推进了“等等
...
- 原则描述：尽量具体，并提供参考示例
  ...
  **Good Example**：“评估需要基于证据事实，Example：步骤计划描述中，步骤B需要依赖步骤A的输出字段`price`，字段`price`的值必须是浮点数。现在步骤A完成后输出的key results中有名为`price`的字段，同时值为666.666，因此步骤A的输出满足步骤B的输入要求，符合隐式要求条件2”

  **Bad Example**: “结合任务目标，请进行高质量的评估并给出评估结论”

- 流程描述：章节"4.5.3"提供了评估思路，把这个思路具体化到执行细节，把手段和过程说清楚

- 标准描述：只要涉及到**标准**的，必须非常明确和量化，不能笼统表达/模糊表达/产生歧义
...
## 5. 安全性体系建设：把工具能力关进权限边界

可靠性关注的是让Agent在失败后尽量恢复执行，安全性关注的则是让 Agent 在不该执行的时候停下来。对于一个能调用工具、读写文件、执行代码、访问数据源的系统来说，安全性不是附属功能，而是工具系统必须具备的边界条件。

目前ThinkingAgent还没有构建完整的安全治理体系，只实现了面向工具调用链路的基础安全边界，主要从以下三个角度展开：
...
为收口和统一处理上面三方面的逻辑，ThinkingAgent使用代理模式为每个Tool都套了一层Safeguard，由Safeguard对工具调用过程进行统一管控
...
依据某些Tool自身的特点，有些Safeguard还需要定制专属的安全管控逻辑，下面举两个例子：

- **Run Python Tool：建立沙箱机制**

Python 工具的风险在于代码执行可以做很多事情，包括访问文件系统、发起网络请求、消耗大量内存等。因此它的Safeguard需要为代码执行提供一个受限运行环境
...
SQL 工具的主要风险是 SQL 注入、越权访问和非预期的数据修改，因此 SQL Query Tool 的 Safeguard 定制了如下防御逻辑：

- **参数化查询**：工具 Schema 明确要求使用参数化查询（`params` 字段），而不是字符串拼接。工具描述中也明确说明"Use placeholders instead of interpolating values directly"
- **只读限制**：工具描述明确说明只支持读语义操作，实际工具执行时 Safeguard 会检查操作命令是否在白名单内
- **行数限制**：默认最多返回 50 行，最大 1000 行，防止全表扫描导致的性能问题
- **数据脱敏**：返回数据如果涉及敏感字段，Safeguard 会执行脱敏再返回
...
LLM 发起一次工具调用后，如果 Safeguard 发现这次工具调用相关信息没有出现在权限系统的白名单中，会通过 Pipeline Driver 向用户端发送操作确认消息。只有收到用户的二次确认后，这次工具调用才会被允许执行
...
**飞轮建设**这一part因为时间关系，也只做了一些非常基本的设计
...
**知识库**和**用户偏好**的信息源有两个：1. 用户主动编辑 2. Agent自动总结
...
ThinkingAgent 的偏好文件是一个不断追加写入的 JSONL 文件，每条记录包含 `created_at` 时间戳。当文件超过阈值（64KB）时，自动删除最旧的 20 条记录（可配置）。
...
## 7. Benchmark：用端到端任务检验交付质量

本章的测试不是严格意义上的学术 Benchmark，而是面向工程实现的端到端冒烟测试。它的目标不是证明 ThinkingAgent 已经具备通用 Agent 能力，而是检查前文设计的几个关键机制在有限复杂任务中是否能协同工作：任务分析、计划生成、工具调用、上下文管理、质量评估和异常恢复。
...
5个任务每个运行40次，共进行200次独立运行。为了避免把有限样本的结果解释过度，下面的数字只表示在这组任务和当前配置下观察到的运行情况：
...
从这组实验看，ThinkingAgent 在这 5 类样本任务上能够比较稳定地完成端到端闭环，尤其是 JSON 回复解析、工具选择、工具参数填充和推理轨迹控制这几个工程风险点，没有出现大量不可恢复的系统性失败。这个结果对我来说主要验证了一个判断：把可靠性设计前置到 Pipeline、LLM Gateway、Tool Registry、ContextManager 和 QualityEvaluator 等关键节点中，确实可以降低复杂任务执行过程中的失控概率。

但这组测试只能说明系统在有限任务集合上的工程可用性，不能推出“ThinkingAgent 可以稳定处理所有复杂任务”。目前它更像一个阶段性健康检查，而不是一个足够严谨的公开基准。

**局限性**

- 测试任务和实验次数非常有限，并不能证明该 Agent 可以应对所有类型任务，并长时间稳定运行
- 实验输出了成本指标数据(token消耗，调用费用)，但是因为没有对比实验，无法定义什么是“好”，所以上面表格中没有列举成本指标情况
- 目前没有设置 baseline 对照组，因此无法量化每个设计模块对成功率、成本和延迟的独立贡献
- LLM as a Judge 只能作为辅助判断，仍然可能存在误判；人工抽样比例也不够高
- 因为时间有限，目前考察指标还比较粗，未来需要补充更细的失败分类、成本统计、延迟统计和消融实验
...
ThinkingAgent 是我在没有参考业界任何其他 Agent 框架设计的情况下，从零开始做的一次工程尝试。之所以一开始没有急着看别人的答案，是因为我想先用自己的问题意识把系统跑通，再带着实践中的困惑去对照已有方案。这样做不一定高效，但对我理解 Agent 系统的真实复杂度有帮助。

它目前还远谈不上成熟，但已经把一些关键基础打了下来：任务分析、计划生成、工具调用、上下文管理、质量评估、检查点恢复、模型路由和安全边界。这篇文章的初衷也不是重复介绍那些已经被反复讨论的概念，而是记录我在真正实现一个 Agent 框架时，对这些问题的取舍和理解。

在实践过程中还有几点特别的体会：

- LLM Agent的可靠性设计和基础能力搭建要同时进行。从开始就放在一起思考，可以避免后期对系统架构的剧烈冲击
- LLM Agent工程最核心和有价值的部分是如何产生一个高质量的推理轨迹，即我们如何有目的地去控制那台高维度贝叶斯概率机器，而其他技术组成部分则如实观照即可
- 提前做好系统可观测性和必要的辅助分析工具，调理推理轨迹是个细心活
- LLM Agent的实践过程是要真金白银花钱的，因为国外大模型的定价比国内高一个数量级，因此我测试时基本使用的都是国产大模型。我统计了一下，平均跑一个测试任务要花4块多人民币，考虑到中间无数次失败和反复调试，累积起来是一笔可观的费用。想起那些数着token过的夜晚，顿感离职的时候应该拿个N+1礼包
