...
> ThinkingAgent 是一个轻量级，准生产就绪的LLM Agent 框架，支持多 LLM Provider、复杂任务编排、自动故障恢复和长任务断点续跑。本文记录了我在从0到1构建ThinkingAgent过程中的一点原创思考，希望能给其他人带来一些新的视角和思路。
...
<img src="/Users/yuwu/Desktop/AILearning/AgentLearning/NanoAgent/docs/assets/image-20260522070158612.png" alt="image-20260522070158612" style="zoom:50%;" />
...
ThinkingAgent有一个周期性任务负责对用户最近的原始偏好Entries进行总结，系统在任务分析阶段将总结好的用户偏好注入到分析上下文中，Analyzer会基于用户提交的具体任务决定应用哪些偏好，并将其偏好转化到分析报告里的"执行约束","工具匹配","模型路由提示"等信息中
...
ThinkingAgent 使用模板管理系统所需要的所有提示词，再通过模板引擎渲染出最终的提示上下文，提示词模板举例比如:
...
很多讲烂的东西不再赘述，这里提一些实践过程中让我更有感觉的东西

##### 3.3.2.1 从第一性原理出发

我想让Transformer机能够采样出我想要的东西，所以我的一些思考点放在如何利用现代大模型的计算过程，大模型训练方法/训练燃料的特征去反推提示词设计。这个领域本身即可独立成文，本文只举一个例子：**注意力的U形曲线**

- 注意力的首因效应

（1）因为因果掩码导致的注意力结构性偏置，引发越靠近头部的token得到的关注越多（注意此时只是多，还不是大）
...
在ThinkingAgent的Prompt设计中对U形曲线特征的运用，主要体现在：在一次推理上下文的头部会放置任务级，提纲挈领般的重要提示，而尾部会被动态插入预防性的纠偏引导
...
有很多任务不止是要一个推理答案，还需要额外输出一些东西，比如输出一篇文档，我把这些额外输出的产物命名为推理的副作用。在实践中我发现必须显示地为这些副作用设计提示词，尤其具有Self-Relection的推理，Prompt举例：
...
Agent评审的输入是从副作用内容抽取的特征，而不是副作用内容本身，比如一个任务：

让Agent输出一个详细的旅行计划，内容不超过3000字

我需要让ThinkingAgent通过**执行约束**来理解"3000字约束"，它会在Execute阶段去检查自己的输出是否满足字数要求，然后输出一条checklist summary，like "使用shell工具统计文档字数为2988字，满足不超过3000字要求"。Agent评审的输入只需要这个checklist summary，而不是整个文档内容
...
ThinkingAgent被设计成需要处理多步骤和具有一定复杂性任务的Agent，这些任务的处理时间往往都是小时级别，最蛋疼的情况是眼看临门一脚，结果程序挂了

**为什么Agent 任务特别脆弱？**

第一，**执行时间长**：一个复杂的 Agent 任务可能需要几十分钟，期间任何一个环节出问题都会导致整体失败。执行时间越长，遇到问题的概率越高
...
ThinkingAgent的检查点包含6个方面的内容，如下图所示：

【TODO图】

- 任务属性信息：任务意图，原始描述，所属用户等
- 执行计划信息：已经制定好的计划
- 运行时信息：已成功交付的步骤，之前恢复留下的反馈等等
- 推理上下文：已经生成的推理轨迹
- 模型选择器信息：模型的熔断状态等
- 所选工具集：为任务执行匹配的工具
...
ThinkingAgent的每个工具都要遵循相同的接口协议，同时每个工具都会附带一个用于处理工具安全校验工作的Safeguard。对于新增的工具，既支持手动注册，也支持扫描后基于反射机制的自动注册

#### 3.5.3 Anti-Skills

仅代表个人观点，我觉得Skills是没有意义的过度性性产物，Agent想要对外部能力进行更高质量地集成，要么想办法提升大模型本身的能力，要么想办法制造一个好的推理轨迹。本着这样一个意识形态，ThinkingAgent的设计遵循了以下的思路：

- 保持工具简单，同时具备好的场景适应性和能力泛化性，就像Shell
- 所有SOP，外部资料细信息等被纳入到“知识”的范畴；人设，风格等被纳入“用户偏好”的范畴
- 工具的按需加载源于Analyzer和Planner的共同决策（本质基于LLM）
- 知识的按需加载源于检索技术+LLM
- 考虑到时效性，对于单个用户而言，一个人的偏好数据 数据量不会很大，因此采取的策略是定期总结近期偏好（比如最近半年），然后将总结内容（对于上下文窗口的占用很可控）全部加载的策略
...
处理可靠性最麻烦的就是它无处不在，每个环节，每次调用都要考虑。而因为大模型的介入，这个问题变得更麻烦。比如你可能看到系统没有任何显示的异常和报错，但推理轨迹就是陷入局部死循环，就是偏离目标。常规系统面临的可靠性问题Agent要cover，而大模型引发的**推理可靠性**问题，LLM Agent也必须提出对应的处理手段

### 4.1 可靠性设计思路

#### 4.1.1 Pillar-Layer异常处理

<img src="/Users/yuwu/Desktop/AILearning/AgentLearning/NanoAgent/docs/assets/image-20260524061032241.png" alt="image-20260524061032241" style="zoom:50%;" />

在“3.1.3三段式工作循环”部分有提到说这样的程序控制流结构有利于可靠性体系设计，正如上图所示，ThinkingAgent的可靠性处理结构与工作循环结构是相辅相成的。每一个横向的切面关注着Pipeline执行的不同层级，也同时关注它要处理异常的层级；而纵向穿插的数轴代表着贯穿所有层次的推理上下文，推理上下文是独立的，它要思考如何解决自身领域的异常。

每一层尽可能处理本层发生的异常，如果发现本层因为缺失信息不知道怎么处理，就向上方的临近层抛出该异常（**备注**：本节提到的**异常**是一个统称，代表破坏可靠性的所有原因。技术实现的载体可能是错误码，Exception，一个普通对象等等）。不同Level关注的异常，发现机制和恢复手段如下表所示：

| Level         | 关注点                                                       | 发现机制                                                     | 恢复手段                                                     |
| ------------- | ------------------------------------------------------------ | ------------------------------------------------------------ | ------------------------------------------------------------ |
| Task          | Task最后交付的结果是否可靠                                   | - Evaluator主动评估<br>- 下层返回的异常                      | - Retry原计划<br>- 重新制定计划并执行                        |
| Stage         | - Stage处理流程是否满足规定的约束<br>- Stage处理流程是否匹配任务整体目标<br>- Stage的交付结果是否可靠 | - Evaluator评估<br>- 下层返回的异常                          | - 原Stage直接重试<br>- 本Stage重新规划并执行<br>- 本Stage即后续Stage重新规划并执行<br>- 重新制定整个计划并执行 |
| Reasoning     | 一次推理动作是否正常                                         | 下层返回的异常                                               | 模型切换                                                     |
| LLM Gateway   | 关注与不同厂商API交互的异常                                  | - 本地检查LLM返回包的完整性<br>- 不同厂商API返回的错误信息   | - 本地修复LLM回包<br>- 带Jitter的Backoff<br>- 错误注入上下文由LLM自行处理<br>- 模型降级 |
| Tool Registry | 工具调用的异常                                               | - 调用前检查<br>- 连续重复调用纠偏<br>- 具体工具返回的错误信息 | - 带Jitter的Backoff<br>- 自动插入纠偏Prompt<br>- 工具降级<br>- 错误注入上下文由LLM自行处理 |
| Context       | 推理轨迹的可靠性                                             | 获取本轮推理所需上下文的一系列检查                           | 上下文修复/压缩/裁剪                                         |
...
- 错误码的展示逻辑与恢复逻辑分离，调用端如果需要记录日志，trace这些就用code/message等只读信息；而要恢复异常则使用caller action和action data；甚至有的简单的异常在ThinkingAgent中直接表驱动处理掉了
...
除了一些特别hard的异常（比如耗尽任务重试次数），其他大部分异常发生时我们都希望能让Agent的执行回归正常的推理轨迹。而不同的恢复策略关乎到token消耗，关乎到时间效率。所以带着代价的意识去思考如何进行异常恢复是自然而然的，这里面暗含了两个子问题：a. 要设计一个基于代价的带优先级的恢复策略(从最低代价开始尝试恢复) （2）恢复策略需要能自动升级（低代价方式恢复不了时必须升级）
...
这样朴素的道理引导了ThinkingAgent异常恢复策略设计，我会在提示词中注入恢复策略，触发条件，以及引导LLM优先选择代价最低的

另一个点就是恢复策略需要有自动升级机制，防止死循环，避免“死磕”。比如ThinkingAgent发现某步骤的重试次数已经超过单步骤最大执行次数限制，就会在推理上下文中自动插入引导语，表达思想是：“你已经进行了XX次重试，超过了系统最大的重试次数要求，现在需要从其他恢复策略中选择一个你认为最合适的。同样需要选择代价更低的那个恢复策略......”。注意这是提示词思想，真正的提示词内容不能出现“最合适的”这种模糊性表达

#### 4.1.4 低置信度澄清

在ThinkingAgent里面除了LLM-in-the-loop，也同时建设了Human-in-the-loop。当面对复杂任务时，人的介入和协同是无可避免的。而回到“可靠性处理”这里，当ThinkingAgent发现置信度过低，需要用户二次确认和澄清时，会主动发送消息给CLI端，并有限等待用户的澄清信息。低置信度澄清在ThinkingAgent的任务分析，计划制定，计划执行，结果评估阶段都有可能发生。
...
ThinkingAgent上下文压缩处理的输入不是原生的基于role的消息，而是要先预处理上下文，将消息拆解成不同的语义块(BLOCK)和语义组(GROUP)

<img src="/Users/yuwu/Desktop/AILearning/AgentLearning/NanoAgent/docs/assets/image-20260525174906542.png" alt="image-20260525174906542" style="zoom:50%;" />

- 语义块(BLOCK)：代表不可再分割的原子语义单元，比如tool use/tool result消息必须做好配对并放在一个语义块中。ThinkingAgent中的语义块BLOCK有SYS块，USER块，ASSIATANT块和TOOL块四种，它们分别对应system prompt，用户prompt，独立的assistant消息和工具调用和回复消息

- 同属于一个Stage的BLOCK被划分成一个GROUP

  将消息分成BLOCK和GROUP是为了确定压缩处理算法的数据边界，这个边界保护了上下文中一个“语言单元”语义的完整性
...
| #    | 策略                     | 含义                                                         | Notes                                                |
| ---- | ------------------------ | ------------------------------------------------------------ | ---------------------------------------------------- |
| 1    | 消除重复的工具调用消息   | 同样的工具和同样的参数连续调用多次，只保留最近的一次         | 在推理轨迹中插入引导信息：<br>表明重复情况和规避建议 |
| 2    | 简化工具调用失败消息     | 去掉工具调用失败的回复细节，修改为一句笼统的表达失败的消息   |                                                      |
...
这个推理轨迹的消息分配和布局有几个点值得注意：

- 使用 Tool Call消息Highlight任务的执行计划
- 重写后的结构化Task Description使用user role消息呈现
- Agent执行过程中用户主动发送的互动消息，Agent主动的纠偏引导消息都使用了user role
- 一个Tool Call Block可能包含不止一次工具调用，调用次数取决于LLM回复。如果出现连续工具调用，则会把一个assistant role的call消息和多个tool role的回复封装到一个BLOCK里
- 每个Stage开始执行时首先会插入一个user role消息来说明本Stage的执行目标/关键产出/执行约束等信息以及再次强调任务整体的目标/用户的真实意图

一个推理轨迹就是与LLM一次完整对话的过程，ThinkAgent想将这个对话塑造成：

1. 从HighLevel常规的角色话术开始，到描述本次任务各维度的信息 ，再到任务整体的规划，单个步骤具体执行策略 ，直到交付任务最后的产出。从整体到局部，逐级细化和递推的过程
2. 在当前步骤的执行前再次callback任务整体的目标，用户真实意图等全局性信息，避免LLM失去全局观
...
```python
下面描述压缩处理过程的简化描述，需要参考上图的一些定义
    Step1: 计算剩余预算=（总预算-Group0的token消耗-当前处理过程中的Group的token消耗-给LLM回复预留的token预算）* 0.7[0.3是给可能的摘要输出预留]
    Step2: 除了Group0和当前正在处理的Group，对其他BLOCK消息都运行策略1，执行后检查是否满足预算，如果在预算内就提前退出
    Step3: 从最后一个Group（图中示意的Group N）的最后一个BLOCK开始倒序扫描，目的是看剩余预算还能保住几个BLOCK。找到开始不能满足预算的第一个BLOCK作为分割点(图中红色虚线箭头)
    Step3: 划分分割点所在的GROUP和其上的GROUPS为“可处理GROUPS”区域，划分分割点所在的GROUP以下的GROUPS为“预算内GROUPS”区域。对可处理GROUPS内的所有BLOCK依次实施策略2/3/4，每个策略实施后检查是否预算情况，如果在预算内就提前退出
    Step4: 对可处理的GROUPS内**除了Stage Description首BLOCK外的其他BLOCK进行摘要总结（策略5），将其压缩成一个BLOCK消息。每完成一个GROUP就检查预算是否满足要求，满足就提前退出
    Step5: 对预算内的所有GROUPS运行策略2/3/4，每个策略实施后检查是否预算情况，如果在预算内就提前退出
    Step6: 对预算内的所有GROUPS，像Step4一样实施策略5。检查是否预算情况，如果在预算内就提前退出
    Step7: 如果还是超预算，就重新规划整个任务，全部重来（出现这种极端情况的概率非常低）
```
...
- 渐进式处理——从侵入最少的策略开始，从可安全处理的区域开始，一旦发现目前窗口中token数满足预算要求就早停，越少处理越少破坏
- 不引入直接裁剪策略，因为在实践中发现这种方式可能引发复杂任务的推理轨迹断裂或者歧义，效果不好

#### 4.2.6 Prompt Caching

部分LLM提供商，比如Anthropic, MiniMax, Kimi等等，它允许API调用方显示地主动控制Transformer的KV Cache。这个Prompt Caching技术对于ThinkingAgent这种需要处理多轮次，多步骤，长程任务的Agent来说非常友好，可以节省不少开支。目前ThinkingAgent支持了使用cache_control参数控制缓存的模式。
...
上图展示了从推理上下文LLM Context到一个具体LLM服务提供商的API Request，以及反向从具体LLM服务提供商的API Response到LLM Context的转化过程。ReasoningManager负责LLM Context与Unified LLM Requst/Response的解析与反解析；LLMGateway使用Unifeid LLM Request调用一个具体的LLM服务器提供商API，由对应的LLM Provider适配器将其转化成服务提供商API具体的Request。收到服务提供商的API回复后再由LLM Provider适配器将回复转成Unified LLM Reponse或者映射错误信息到标准错误码，进而推动LLMGateway接下来的处理。
...
所有LLM调用的回复都会被解析和转化成`UnifiedLLMResponse`
...
| 2    | Model Degrade         | Quota Exceeed<br>触发Content Policy<br>解析问题<br>......    |
| 3    | Model Provider Switch | 4xx Auth/Permission<br>Content Length<br>Invalid Request<br>.......... |

##### 4.3.3.2 报文内容修复

有时没有显示的错误码指示，但是LLM Provider回复的报文内容不合法，主要有以下两种情况：

- 格式非法：比如Agent需要JSON格式，但是LLM返回了JSON fence
- 丢失必要的key-value

如果返回内容没有丢失关键的key-value，只是格式非法，会先用本地JSON schema修复工具处理成合法JSON，继续回到正确的推理轨迹中。如果内容缺失关键信息，Agent会插入引导提示词，让LLM尝试自己修复

##### 4.3.3.3 总结

LLMGateway本身没有特别多的内容，它的重点是设计统一标准协议，对接不同LLM服务提供商的API。目前ThinkingAgent对接了OpenAI，Anthropic，DeepSeek，GLM，Kimi，Qwen，MiniMax的大模型
...
- 提升效率：有些工具调用是比较重的，比如拉起解释器
- 逻辑解耦：准入检查不过需要执行的应对逻辑，都会收口在Tool Registry层，Tool就负责执行工具逻辑。
...
工具调用返回错误，如果不能采用降级处理的手段，Agent会将错误信息注入到推理上下文中，由LLM决定下一步如何处理
...
对一个执行计划质量的评估会从三个阶段展开：

- 评估任务与计划匹配情况：比如“需要严格遵守的执行约束是否纳入了计划步骤中“，“输出约束在最后一步的关键结果或者执行上是否体现出来”等等
- 从因看果：从第一个步骤开始检查，直到最后一个步骤。检查内容包含三个方面：a. 基于当前步骤所描述的输入和步骤过程能否产生预期的输出；b. 这个步骤的输出是否满足依赖它的其他步骤对于输入的要求；c. 最后一步的输出是否满足任务目标。这就像检查多米诺骨牌能否正常都倒下：每个牌能否正常倒下去 -> 前面的牌倒下去能否推地动下一个牌 -> 最后一个牌倒下去是否满足整体目标
- 由果观因：从最后一个步骤开始检查，直到第一个步骤。检查内容包含：a.基于当前步骤的过程和预期结果，当前步骤描述的输入是否满足过程要求；b. 当前步骤需要的输入，其依赖的前置步骤是否能明确提供；c.第一个步骤的输入可以从任务的原始输入(任务目标，描述，关键实体信息等等)中获取

实践中发现通过“全局静态匹配”+“双向因果关系论证”的评估思路可以让任务执行的成功率提升～15个百分点

（2）**阶段结果评估**

对一个阶段结果的评估主要从显式要求和隐式要求两个方面展开：

- 显式要求：“计划步骤要求的key results，真正执行的结果里是否有包含”；“步骤输出的结果满足输出约束要求吗”等等
- 隐式要求：“结果是否按照 `output_constraints` 的描述生成了对应的数据对象，并且其格式能让后续步骤直接使用？“；”结果是否真正实现了步骤目标，而不仅仅是部分达成？同时，验证结果是否推动整体 `task_goal` 的进展“等等
...
我们可以将所有描述划分为三类，如下图所示：

<img src="/Users/yuwu/Desktop/AILearning/AgentLearning/NanoAgent/docs/assets/image-20260527053607999.png" alt="image-20260527053607999" style="zoom:50%;" />

- 原则描述：尽量具体，并提供参考示例

  **Good**：评估需要基于证据，例子：步骤计划描述中，步骤B需要依赖步骤A的输出字段`price`，字段`price`的值必须是浮点数。现在步骤A执行输出的key results中有名为`price`的字段，同时值为666.666，因此步骤A的输出满足步骤B的输入要求，符合隐式要求条件2

  **Bad**: 结合任务目标，请进行高质量的评估并给出评估结论

- 流程描述：策略和流程描述如章节"4.6.3"所述，要把评估的过程，怎么评估说清楚

- 标准描述：只要涉及到**标准**的，必须非常明确和量化，不能笼统/模糊/有歧义

  **Good**: 对于执行计划中要求的所有key results，如果步骤执行后的输出缺少key results中任意一个字段，这个步骤执行的结果应该被视为严重错误，不能评审通过，需要执行恢复步骤......

  **Bad**: 对步骤的执行结果进行评估，如果遇到严重错误，需要执行恢复步骤.....
...
- **高危操作需用户二次确认**
  ...
  为收口和统一处理上面三方面的逻辑，ThinkingAgent使用代理模式为每个Tool都套了一个Safeguard"保护壳"，由Safeguard对Tool调用全生命周期活动进行安全管控

<img src="/Users/yuwu/Desktop/AILearning/AgentLearning/NanoAgent/docs/assets/image-20260527210719122.png" alt="image-20260527210719122" style="zoom:50%;" />

Safeguard实施管控的权限信息输入来自两个地方，一个是权限系统中的配置，另一个是用户的二次确认信息

### 5.1 Safeguard通用检查

所有工具的Safeguard都会共享一些通用的检查和处理逻辑：

- 检查用户的工具白名单，是否允许当前工具调用
- 工具的参数进行类型、范围、格式校验
- 工具调用频率限制
- 为每个会话任务创建独立的工作子目录，将工具增删改查的数据做到会话级别隔离
...
代码在独立的子进程中执行（使用 `multiprocessing.Process`），并通过 `resource` 模块设置内存上限和 CPU 时间上限。即使代码绕过了前两层防御，资源限制也能防止它消耗过多系统资源
...
- **库表限制**:   通过权限系统的配置感知能够访问哪些库表

### 5.3 高危操作的用户二次确认

如果LLM返回的Tool Call操或输入参数，没有出现权限系统的配置中，Safeguard会通过PipelineDriver向用户端发送操作确认消息，收到用户的二次确认后才能继续执行
...
知识库在任务开始执行时被加载，注入到任务上下文中，让 LLM 在推理时可以利用这些已知信息

偏好库在任务分析和执行阶段都会被查询，注入到系统提示中，影响 LLM 的整体行为风格
...
在任务完成并通过评估，交付前，Agent会**异步**地提取知识和偏好——在后台线程中执行，不阻塞当前任务的返回
...
（4）任务需要有明确的限制和约束条件，考验Agent输出的结果是否满足约束
...
5个任务每个运行40次，共运行200次完全独立的实验，实验情况如下表所示：

| #    | 考察指标                          | 判定方式                                  | 实验结果 |
| ---- | --------------------------------- | ----------------------------------------- | -------- |
| 1    | 推理轨迹正确率                    | 全量LLM as a Judge + 人工随机抽样10%      | 99%      |
| 2    | 工具选择正确率                    | 全量LLM as a Judge + 人工随机抽样10%      | 98%      |
| 3    | LLM回复JSON结构解析准确率         | 程序报错监控                              | 100%     |
| 4    | 任务完成率                        | 人工评审                                  | 100%     |
| 5    | 工具参数填充正确率                | 非语义层面程序上报+语义层面LLM as a Judge | 98.75%   |
| 6    | 轨迹效率(最少必要步数 / 实际步数) | 全量LLM as a Judge                        | Avg 0.75 |

**额外说明**

- 测试任务和实验次数非常有限，并不能证明该Agent可以应对所有类型任务，并长时间稳定运行
- 有成本指标数据(token消耗，调用费用)，但是没有对比，无法定义什么是“好”，所以上述结论没有给出
- 考察指标的设计非常单薄，未来需要迭代和补充

## 8. 结语

ThinkingAgent是我给自己造的一个玩具，没有参考任何业界其他Agent的设计，完全以自己的想法作为基础，边做边思考边落地的产物。之所以没有上来就参考其他Agent，是因为我想带着答案去看别人的答案，而不是带着问题去看别人的答案。每一个设计和决策都有它的理由，没有痛过是无法深入理解的。同样的问题，为什么别人选择了一条与我不同的，更好的道路？通过这种对比思考，我才能可以感受不同设计之间的差距，弄清楚自己思维和知识的盲点。所以ThinkingAgent目前肯定是非常幼稚的，但是我觉得打下了一个不错的基础

在实践过程中还有几点特别的体会：

- LLM Agent的可靠性设计和基础能力搭建要同时进行。从开始就放在一起设计，可以避免后期对系统架构的剧烈冲击
- LLM Agent工程最核心和有价值的部分是如何产生一个好的推理轨迹，即我们如何有目的地去控制那台高维度贝叶斯概率机器，而其他的技术组成部分如实观照即可
- 提前做好系统可观测性和制造必要的辅助分析工具，调理推理轨迹强依赖这些东西

- LLM Agent的实践过程是要真金白银花钱的，因为国外大模型的定价比国内高一个数量级，因此我测试时基本使用的都是国产大模型。我统计了一下，平均跑一个测试任务要花1块多人民币，考虑到中间无数次的失败，累积起来就是一笔可观的费用。想起那些数着token过的日子，顿感离职的时候应该拿个N+1
