...
| No.  | 选项所属菜单等级 | 菜单选项           | 选项说明                           | 用户选择该指令的条件                  | 下一级菜单选项有哪些                                         |
| ---- | ---------------- | ------------------ | ---------------------------------- | ------------------------------------- | ------------------------------------------------------------ |
| 1    | 一级菜单         | [1] New Task       | 提交新任务                         | 1. 用户在一级菜单 2.输入以"#1"开头    | 进入二级菜单，包含选项No. 2,3,4,5                            |
| 2    | 二级菜单         | [1] Input          | 用户选择手动输入任务描述           | 1. 用户在二级菜单 2.输入以"#1"开头    | 停留在二级菜单 No. 2,3,4,5。用户此时可以输入任务描述         |
| 3    | 二级菜单         | [2] Upload by File | 用户选择提供包含任务描述的文件路径 | 1. 用户在二级菜单 2. 输入以"#2"开头   | 停留在二级菜单 No. 2,3,4,5。用户此时可以输入包含任务描述的文件路径 |
| 4    | 非一级菜单都可以 | [b] Back           | 用户选择回退到上一级菜单           | 1. 用户在非一级菜单 2. 输入以"#b"开头 | 返回上一级菜单，比如当前在二级菜单，用户输入"#b"，则回到一级菜单，并展示一级菜单所有选项 |
| 5    | 每一级菜单都可以 | [q] Quit           | 用户想要直接退出整个程序           | 1. 用户在任意一级菜单 2输入以"#q"开头 | 直接退出程序                                                 |
| 6    | 一级菜单         | [2] Cancel         | 用户取消当前任务执行               | 1. 用户在一级菜单 2.输入以"#2"开头    | 停留在一级菜单                                               |
| 7    | 一级菜单         | [3] Guidance       | 用户主动发送任务执行建议           | 1. 用户在一级菜单 2.输入以"#3"开头    | 停留在一级菜单。用户此时可以输入指引信息                     |
| 8    | 一级菜单         | [4] Clarify        | 用户提交澄清信息                   | 1. 用户在一级菜单 2.输入以"#4"开头    | 停留在一级菜单。用户此时可以输入澄清信息                     |
| 9    | 一级菜单         | [5] Resume         | 用户要求任务继续执行               | 1. 用户在一级菜单 2.输入以"#5"开头    | 停留在一级菜单                                               |

一定要知道当前的菜单等级，然后根据用户输入决策用户选择的指令。用户的输入提示部分，格式始终为Thinking> "用户实时输入内容"
